...
| `integral_gain` | `0.1` | `:47` | `:43` | ILOS 적분게인 \(\kappa\) (leaky integrator) | 크면 누적 횡오차 보정이 빨라짐, 과하면 진동 |
...
| `cruise_speed` | `1.0` | `:61` | `:17` | 직선 구간 주행속도(m/s, 약 2knot) | 직선 목표 전진속도 |
| `min_speed` | `0.3` | `:62` | `:29` | 커브 최소속도(m/s) | 커브에서 떨어지는 속도의 하한 |
| `curvature_gain` | `3.0` | `:63` | `:23` | 곡률 기반 속도 저감 계수 | 크면 커브에서 더 많이 감속 |
...
| `adaptive_lookahead` | `False` | `:72` | `:39` | 곡률 기반 동적 전방주시 | `true`면 곡률에 따라 \(\Delta\)를 자동 조정 |
