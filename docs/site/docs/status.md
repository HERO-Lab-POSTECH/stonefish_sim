...
| # | 이슈 | 영향 | 상태 |
|---|------|------|------|
| 1 | 하이브리드 제어기 YAML과 코드의 `max_force`·`max_torque` 기본값 불일치 | 위치 모드 일부 기본값이 YAML(`800`/`160`)과 코드에서 어긋남 | 해소됨 — fix/thrust-map에서 전 모드 YAML·코드 기본값을 물리 한계 55 N/13.7 N·m로 동기 |
