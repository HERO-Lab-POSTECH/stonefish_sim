...
| `start_thruster_manager` | `true` | thruster manager 기동 여부 |

근거: bringup.launch.py:35-54 (DeclareLaunchArgument: namespace, use_sim_time, vehicle, start_control, start_path, start_thruster_manager).

!!! note "bringup.launch.py는 `scenario` 인자가 없다"
    `bringup.launch.py`는 차량 단위로 스택을 기동하며 시나리오 인자를 받지 않는다(시나리오는 vehicle launch 내부 기본값으로 결정). 시나리오를 바꿔 띄우려면 시뮬레이터 launch에서 `scenario:=`를 지정한다.
    ```bash
    ros2 launch stonefish_ros2 bluerov2.launch.py scenario:=bluerov2_terrain
    ```
...
| `cruise_speed` | `1.0` | 직선 주행속도 m/s |
| `min_speed` | `0.3` | 커브 최소속도 m/s |
| `curvature_gain` | `3.0` | 곡률 기반 속도저감 계수 |
| `integral_gain` | `0.1` | ILOS 적분게인 κ |
| `use_alos` | `False` | `True`면 ALOS, `False`면 ILOS |
| `adaptive_lookahead` | `False` | 곡률 기반 동적 전방주시 |
