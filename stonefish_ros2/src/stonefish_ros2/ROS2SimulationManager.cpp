...
void ROS2SimulationManager::SimulationStepCompleted(Scalar timeStep)
{
    ...
    for(size_t i=0; i<rosRobots_.size(); ++i)
    {
           ...
           && pubs_.find(rosRobots_[i]->robot_->getName() + "/thrusters") != pubs_.end())
        {
            ...
            Actuator* actuator;
            Thruster* th;
            stonefish_msgs::msg::ThrusterState msg;
            msg.header.stamp = rclcpp::Clock(RCL_ROS_TIME).now();
            msg.header.frame_id = rosRobots_[i]->robot_->getName();
...
void ROS2SimulationManager::FLSService(const stonefish_msgs::srv::SonarSettings::Request::SharedPtr req,
                                    stonefish_msgs::srv::SonarSettings::Response::SharedPtr res, FLS* fls)
{
...
void ROS2SimulationManager::SSSService(const stonefish_msgs::srv::SonarSettings::Request::SharedPtr req,
                                    stonefish_msgs::srv::SonarSettings::Response::SharedPtr res, SSS* sss)
{
    if(req->range_min <= 0 || req->range_max <= 0 || req->gain <= 0 || req->range_min >= req->range_max)
    {
        res->success = false;
...
void ROS2SimulationManager::MSISService(const stonefish_msgs::srv::SonarSettings2::Request::SharedPtr req,
                                    stonefish_msgs::srv::SonarSettings2::Response::SharedPtr res, MSIS* msis)
{
