...
#include "stonefish_msgs/msg/int32_stamped.hpp"
#include "stonefish_msgs/msg/beacon_info.hpp"
#include "stonefish_msgs/msg/dvl.hpp"
#include "stonefish_msgs/msg/ins.hpp"
#include "stonefish_msgs/msg/thruster_state.hpp"
...
bool ROS2ScenarioParser::ParseRobot(XMLElement* element)
{
    ...
    if((item = element->FirstChildElement("ros_publisher")) != nullptr)
    {
        ...
        if(nThrusters > 0 && item->QueryStringAttribute("thrusters", &topicThrust) == XML_SUCCESS)
            pubs[robot->getName() + "/thrusters"] = nh_->create_publisher<stonefish_msgs::msg::ThrusterState>(std::string(topicThrust), 10);
...
bool ROS2ScenarioParser::ParseAnimated(XMLElement* element)
{
    ...
    else //State of the trajectory published with a message
    {
        ...
        pubs[nameStr + "/iteration"] = nh_->create_publisher<stonefish_msgs::msg::Int32Stamped>(std::string(topic) + "/iteration", 10);
    }
        ...
        switch(sens->getType())
        {
            ...
            case SensorType::LINK:
            {
                switch(((ScalarSensor*)sens)->getScalarSensorType())
                {
                    ...
                    {
                        pubs[sensorName] = nh_->create_publisher<stonefish_msgs::msg::DVL>(topicStr, queueSize);
                    ...
                    {
                        pubs[sensorName] = nh_->create_publisher<stonefish_msgs::msg::INS>(topicStr, queueSize);
            ...
            case SensorType::VISION:
            {
                switch(((VisionSensor*)sens)->getVisionSensorType())
                {
                    ...
                    case VisionSensorType::MSIS:
                    {
                        ...
                        std::function<void(const stonefish_msgs::srv::SonarSettings2::Request::SharedPtr req, 
                            stonefish_msgs::srv::SonarSettings2::Response::SharedPtr res)> callbackFunc =
                        std::bind(&ROS2SimulationManager::MSISService, sim, _1, _2, msis);                    
                        srvs[sensorName] = nh_->create_service<stonefish_msgs::srv::SonarSettings2>(topicStr + "/settings", callbackFunc);
                        img_pubs[sensorName] = it->advertise(topicStr + "/image", queueSize);
                        img_pubs[sensorName + "/display"] = it->advertise(topicStr + "/display", queueSize);
                    }
                        break;
    ...
    if(comm != nullptr)
    {
        ...
        switch(comm->getType())
        {
            case CommType::USBL:
            {
                ...
                pubs[commName + "/beacon_info"] = nh_->create_publisher<stonefish_msgs::msg::BeaconInfo>(topicStr + "/beacon_info", 10);
            }
                break;
