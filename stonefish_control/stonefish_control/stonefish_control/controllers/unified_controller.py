...
class InnerLoopController:
    ...
    Control Law:
        tau = Kp × e_vel + Ki × integral(e_vel) + M × a_ff + tau_gravity
    ...
    Phase 2.1 Enhancement:
        Added gravity/buoyancy feedforward for heave axis compensation.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            weight: Vehicle weight in water (N) - Phase 2.1
            buoyancy: Buoyancy force (N) - Phase 2.1
            gravity_compensation: Enable gravity/buoyancy feedforward - Phase 2.1
        """
        self.Kp = np.asarray(Kp, dtype=np.float64)
        ...
        # Phase 2.1: Gravity/Buoyancy parameters
        self._weight = weight      # Weight force (N)
        self._buoyancy = buoyancy  # Buoyancy force (N)
    ...
    def compute(
        ...
        # Phase 2.1: Gravity/Buoyancy feedforward
        # In NED frame: Weight acts in +Z (down), Buoyancy in -Z (up)
        # Net force = Weight - Buoyancy (positive = sinks, negative = floats)
        # For 4DOF: [surge, sway, heave, yaw] → gravity only affects heave (index 2)
        gravity_term = np.zeros(4)
        if self._gravity_compensation:
            delta_buoyancy = self._weight - self._buoyancy  # Net downward force
            gravity_term[2] = delta_buoyancy  # Apply to heave axis

        # Total control (before saturation)
        u_raw = p_term + i_term + ff_term + gravity_term
        ...
        info = {
            ...
            'i_term': i_term,
            'ff_term': ff_term,
            'gravity_term': gravity_term,  # Phase 2.1
            'u_raw': u_raw,
            'u_sat': u_sat,
...
class Unified6DOFController:
    ...
    Phase 2.2 Enhancement:
        Bumpless transfer for smooth mode switching via velocity command blending.
    """
    ...
    def __init__(
        ...
        bumpless_blend_duration: float = 0.5,
        weight: float = 197.7,
        buoyancy: float = 197.7,
        gravity_compensation: bool = True
    ):
        ...
        Args:
            ...
            bumpless_blend_duration: Blend duration for mode switch (s) - Phase 2.2
            weight: Vehicle weight (N) - Phase 2.1
            buoyancy: Buoyancy force (N) - Phase 2.1
            gravity_compensation: Enable gravity feedforward - Phase 2.1
        """
        # Store parameters
        self._gains = gains
        self._limits = limits
        ...
        # Create inner loop controller (Phase 2.1: with gravity compensation)
        ...
        # Phase 2.2: Bumpless transfer state
        self._bumpless_blend_duration = bumpless_blend_duration
        self._vel_cmd_at_switch = np.zeros(4)  # Velocity at mode switch
        self._blend_elapsed = 0.0  # Elapsed time since mode switch
        self._blending_active = False  # Blend in progress flag
    ...
    def set_mode(self, mode: str, current_vel_cmd: Optional[np.ndarray] = None) -> None:
        """Set control mode with bumpless transfer.

        Args:
            mode: Control mode ('position', 'velocity', 'hybrid')
            current_vel_cmd: Current velocity command for bumpless transfer (optional)

        Note:
            Phase 2.2: Mode switch initiates velocity blending for smooth transition.
            Inner loop is reset but velocity command is blended over blend_duration.
        """
        new_mode = ControlMode(mode)

        if new_mode != self._mode:
            # Phase 2.2: Bumpless transfer - save current velocity command
            if current_vel_cmd is not None:
                self._vel_cmd_at_switch = current_vel_cmd.copy()
            else:
                self._vel_cmd_at_switch = np.zeros(4)

            # Start blending
            self._blend_elapsed = 0.0
            self._blending_active = True
    ...
    def compute(
        ...
        # Phase 2.2: Bumpless transfer - blend velocity commands after mode switch
        if self._blending_active:
            self._blend_elapsed += dt

            if self._blend_elapsed < self._bumpless_blend_duration:
                # Linear blend from old velocity to new velocity
                alpha = self._blend_elapsed / self._bumpless_blend_duration
                vel_cmd_new = vel_cmd.copy()
                vel_cmd = (1.0 - alpha) * self._vel_cmd_at_switch + alpha * vel_cmd_new
                info['bumpless_blend'] = {
                    'active': True,
                    'alpha': alpha,
                    'vel_old': self._vel_cmd_at_switch.tolist(),
                    'vel_new': vel_cmd_new.tolist()
                }
            else:
                # Blending complete
                self._blending_active = False
                info['bumpless_blend'] = {'active': False, 'completed': True}
    ...
    def reset(self) -> None:
        ...
        self._inner_loop.reset()
        self._mode_switch_count = 0
        # Phase 2.2: Reset bumpless transfer state
        self._vel_cmd_at_switch = np.zeros(4)
        self._blend_elapsed = 0.0
        self._blending_active = False
