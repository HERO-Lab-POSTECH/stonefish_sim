...
class LinearInterpolator(PathGenerator):
    ...
    ```python
    from stonefish_trajectory_manager import Waypoint, WaypointSet, LinearInterpolator
    ...
    def get_samples(self, max_time, step=0.001):
        ...
        * `step` (*type:* `float`, *default:* `0.001`): Step size in parametric space
        ...
        for i in s:
            ...
            pnt.rotq = self.generate_quat(i)  # FIX: Add quaternion calculation!
            pnt.t = 0.0
            pnts.append(pnt)
        return pnts
