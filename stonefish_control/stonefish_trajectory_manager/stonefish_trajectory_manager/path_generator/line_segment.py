...
class LineSegment(object):
    ...
    * `p_target` (*type:* `list` or `numpy.array`): Line's ending point
    ...
    def interpolate(self, u):
        """Interpolate the line segment using the input parametric variable `u`.
        ...
        * `u` (*type:* `float`): Curve parametric input in the interval `[0, 1]`
        ...
        `numpy.array`: 3D point from the line segment
        """
        u = max(u, 0)
        u = min(u, 1)
    ...
    def get_derivative(self, *args):
        ...
        `numpy.array`: 3D derivative value from the line segment
        """
