...
#include <stonefish_msgs/DVL.h>
#include <stonefish_msgs/INS.h>
#include <stonefish_msgs/ThrusterState.h>
#include <stonefish_msgs/Int32Stamped.h>
#include <stonefish_msgs/BeaconInfo.h>
#include <stonefish_msgs/Event.h>
#include <stonefish_msgs/EventArray.h>
...
bool ROSScenarioParser::ParseRobot(XMLElement* element)
{
    ...
    if((item = element->FirstChildElement("ros_publisher")) != nullptr)
    {
        ...
        if(nThrusters > 0 && item->QueryStringAttribute("thrusters", &topicThrust) == XML_SUCCESS)
            pubs[robot->getName() + "/thrusters"] = nh.advertise<stonefish_msgs::ThrusterState>(std::string(topicThrust), 10);
...
bool ROSScenarioParser::ParseAnimated(XMLElement* element)
{
    ...
    else //State of the trajectory published with a message
    {
        ...
        pubs[nameStr + "/iteration"] = nh.advertise<stonefish_msgs::Int32Stamped>(std::string(topic) + "/iteration", 10);
    }
        ...
        switch(sens->getType())
        {
            ...
            case SensorType::LINK:
            {
                switch(((ScalarSensor*)sens)->getScalarSensorType())
                {
                    ...
                    {
                        pubs[sensorName] = nh.advertise<stonefish_msgs::DVL>(topicStr, queueSize);
                    ...
                    {
                        pubs[sensorName] = nh.advertise<stonefish_msgs::INS>(topicStr, queueSize);
            ...
            case SensorType::VISION:
            {
                switch(((VisionSensor*)sens)->getVisionSensorType())
                {
                    ...
                    {
                        pubs[sensorName] = nh.advertise<stonefish_msgs::EventArray>(topicStr, queueSize);
                    ...
                    case VisionSensorType::FLS:
                    {
                        ...
                        srvs[sensorName] = nh.advertiseService<stonefish_msgs::SonarSettings::Request, stonefish_msgs::SonarSettings::Response>(topicStr + "/settings", FLSService(fls));
                    ...
                    case VisionSensorType::SSS:
                    {
                        ...
                        srvs[sensorName] = nh.advertiseService<stonefish_msgs::SonarSettings::Request, stonefish_msgs::SonarSettings::Response>(topicStr + "/settings", SSSService(sss));
                        img_pubs[sensorName] = it.advertise(topicStr + "/image", queueSize);
                        img_pubs[sensorName + "/display"] = it.advertise(topicStr + "/display", queueSize);
                    }
                        break;
                    ...
                    case VisionSensorType::MSIS:
                    {
                        ...
                        srvs[sensorName] = nh.advertiseService<stonefish_msgs::SonarSettings2::Request, stonefish_msgs::SonarSettings2::Response>(topicStr + "/settings", MSISService(msis));
                        img_pubs[sensorName] = it.advertise(topicStr + "/image", queueSize);
                        img_pubs[sensorName + "/display"] = it.advertise(topicStr + "/display", queueSize);
        ...
        switch(comm->getType())
        {
            ...
            case CommType::USBL:
            {
                if(pubTopic != nullptr)
                {
                    ...
                    pubs[commName + "/beacon_info"] = nh.advertise<stonefish_msgs::BeaconInfo>(topicStr + "/beacon_info", 10);
