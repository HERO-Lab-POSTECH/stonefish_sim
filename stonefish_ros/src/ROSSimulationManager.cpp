...
void ROSSimulationManager::SimulationStepCompleted(Scalar timeStep)
{
    ...
    for(size_t i=0; i<rosRobots.size(); ++i)
    {
           ...
           && pubs.find(rosRobots[i]->robot->getName() + "/thrusters") != pubs.end())
        {
            ...
            unsigned int thID = 0;
            Actuator* actuator;
            stonefish_msgs::ThrusterState msg;
            msg.header.stamp = ros::Time(getSimulationTime(true));
            msg.header.frame_id = rosRobots[i]->robot->getName();
...
bool ROSSimulationManager::RespawnRobot(stonefish_msgs::Respawn::Request& req, stonefish_msgs::Respawn::Response& res)
{
...
bool FLSService::operator()(stonefish_msgs::SonarSettings::Request& req, stonefish_msgs::SonarSettings::Response& res)
{
...
bool SSSService::operator()(stonefish_msgs::SonarSettings::Request& req, stonefish_msgs::SonarSettings::Response& res)
{
    if(req.range_min <= 0 || req.range_max <= 0 || req.gain <= 0 || req.range_min >= req.range_max)
    {
        res.success = false;
...
bool MSISService::operator()(stonefish_msgs::SonarSettings2::Request& req, stonefish_msgs::SonarSettings2::Response& res)
{
